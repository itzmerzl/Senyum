        ...
        >
            {stats.length > visibleCards && (
                <>
                    <button
                        onClick={handlePrev}
                        disabled={!canGoPrev}
                        className={`absolute -left-3 top-1/2 -translate-y-1/2 z-10 p-1.5 rounded-full shadow-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 transition-all ${canGoPrev
                    ...
                    <button
                        onClick={handleNext}
                        disabled={!canGoNext}
                        className={`absolute -right-3 top-1/2 -translate-y-1/2 z-10 p-1.5 rounded-full shadow-lg bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-600 transition-all ${canGoNext
                            ? 'opacity-0 group-hover:opacity-100 hover:bg-gray-50 dark:hover:bg-gray-700'
                            : 'opacity-0 cursor-not-allowed'
                            }`}
                    >
            ...
            <div className="overflow-hidden rounded-xl" ref={containerRef}>
                <div
                    ...
                    style={{
                        gap: '12px',
                        transform: `translateX(calc(-${currentIndex} * (100% / ${visibleCards} + 12px / ${visibleCards})))`
                    }}
                >
                    {stats.map((stat, index) => (
                        <div
                            key={index}
                            className="flex-shrink-0 bg-white dark:bg-gray-800 p-4 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-all relative overflow-hidden group"
                            style={{
                                width: `calc((100% - ${(visibleCards - 1) * 12}px) / ${visibleCards})`,
                                minWidth: `calc((100% - ${(visibleCards - 1) * 12}px) / ${visibleCards})`
                            }}
                        >
                            ...
                            <div className="flex justify-between items-start mb-2.5 relative z-10">
                                <div>
                                    ...
                                    <h3 className={`text-xl font-bold mt-0.5 ${stat.valueColor || 'text-gray-900 dark:text-white'}`}>
                                        {stat.value}
                                    </h3>
                                </div>
                                <div className={`p-2 rounded-lg ${stat.iconBg || 'bg-blue-50 dark:bg-blue-900/20'}`}>
                                    {stat.icon && <stat.icon className={`w-4.5 h-4.5 ${stat.iconColor || 'text-blue-600 dark:text-blue-400'}`} />}
                                </div>
                            </div>
